...
### 書き出しサイズ（表示の大きさもここで決まる）
`annotate.lua` の `MAX_WIDTH`（既定 **720px**）で `.ann.png` の最大幅を決める。0 で原寸。
...
### 縮小中に作るマーカーのサイズ補正
文字サイズ・線幅は「画像の座標系」で持つため、50% 表示のまま既定値で作ると画面でも書き出しでも
半分になる（縮小するほど字が小さくなる）。`markercreate` で倍率の逆数を掛けて補正しているので、
**どの倍率で作っても画面で見えている通りの大きさで焼ける**（100%→1rem / 50%→2rem / 25%→4rem、
線幅 3→6）。復元したマーカーには掛けない（作成時の見た目を保つため）。
