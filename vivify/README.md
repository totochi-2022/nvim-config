...
- `lua/diagram.lua` … 回路図(schemdraw)の事前SVG化＋round-trip編集。
  ...
  - **auto描画は既定 OFF**（`vim.g.diagram_autorender = true` で有効化）。auto にするとビュー時に
    バッファがフェンス→`![]` に書き換わり、ブラウザ(端末モードはディスク表示)と行番号がズレて
    **スクロール追従が壊れる**ため。整合させるなら `:DiagramRender`→保存してからプレビュー。
