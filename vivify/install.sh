...
echo "[6/6] figure studio 依存 (streamlit / streamlit-ace / schemdraw / matplotlib)"
if command -v python3 >/dev/null; then
    python3 -m pip install --quiet streamlit streamlit-ace schemdraw matplotlib \
        && echo "  導入OK（:Studio / ,,e で使用）" \
        || echo "  △ 一部失敗。手動: pip install streamlit streamlit-ace schemdraw matplotlib"
else
    echo "  △ python3 が無いので studio 依存スキップ（python 用意後に pip install）"
fi
