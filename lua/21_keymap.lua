...
--- window navigation via F13-F16 (kanata: 無変換+hjkl → F13-F16。物理キーに無い＝他アプリ非干渉)
-- 窓移動: kanata の 無変換+hjkl が「Shift+F1 → 文字」を送る2ストローク。
-- Shift+F1 の届き方が端末で違う(WT=<F13> / ttyd=<S-F1>)ため両プレフィックスを割当。
-- <Cmd>wincmd で ノーマル/挿入/ターミナル 全モード副作用なし。
-- 現在バッファを explorer.exe で開く（<Leader>E と 変換/無変換+e で共用）
local function open_current_in_explorer()
...
-- 作業ディレクトリを explorer.exe で開く（変換/無変換+e）。
-- 端末バッファなら、その端末プロセス(シェル=Claude Code起動元)の実cwdを /proc/<pid>/cwd から取得。
-- それ以外(通常バッファ)は nvim の getcwd() にフォールバック。
local function open_cwd_in_explorer()
    local dir
    local pid = vim.b.terminal_job_pid
    if pid and pid > 0 then
        local target = vim.uv.fs_readlink('/proc/' .. pid .. '/cwd')
        if target and target ~= '' then dir = target end
    end
...
keymap('n', '<Leader>n', '<cmd>ClaudeAttnFocus<CR>', { noremap = true, desc = 'Claude 待ちペインへフォーカス(多画面)' })
keymap('n', '<Leader>N', '<cmd>ClaudeAttnSwitch<CR>', { noremap = true, desc = 'Claude 待ちを現ウィンドウに出す(1画面)' })
...
-- ターミナルモード
-- 単独 <Esc> はターミナルアプリ(claude-code 等の TUI)へ通し、
-- ノーマルモードへ戻るのは <Esc><Esc> の二連打にする。
keymap("t", "<Esc><Esc>", [[<C-\><C-n>]], { desc = 'ノーマルモードへ（ESC二連打）' })
...
-- howm（telekasten）: ,, をプレフィックスにして ,,<key> で各コマンドに直行
-- ,, 単体は割り当てず純粋なプレフィックスにする（which-keyのポップアップが出る）
-- ,,, でコマンドパレット（全コマンド一覧）
...
keymap('n', ',,r', '<cmd>HowmDiaryLatest<CR>',           { noremap = true, desc = 'howm: 最新の日記エントリを開く' })
...
keymap('n', ',,i', '<cmd>Telekasten insert_link<CR>',    { noremap = true, desc = 'howm: リンク挿入' })
keymap('n', ',,b', '<cmd>Telekasten show_backlinks<CR>', { noremap = true, desc = 'howm: バックリンク一覧' })
keymap('n', ',,t', '<cmd>Telekasten show_tags<CR>',      { noremap = true, desc = 'howm: タグ一覧' })
...
-- ターミナルモードで <C-CR>/<S-CR> を「改行(LF)」として送信。
-- 端末では Enter=CR(\r) で、Claude 等の TUI はこれを「送信」と解釈する。
-- LF(\n=Ctrl-J) を送ると「改行の挿入」になるので、複数行入力できる。
-- ※ <C-CR>/<S-CR> が nvim に届くかは端末/web フロント側のキー転送依存。
...
keymap('t', '<C-CR>', term_newline, { desc = 'ターミナルに改行(LF)を送信（Claude等の複数行入力）' })
keymap('t', '<S-CR>', term_newline, { desc = 'ターミナルに改行(LF)を送信（Claude等の複数行入力）' })
